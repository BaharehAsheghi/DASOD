...
class test_dataset:
    def __init__(self, image_root, pseudomask_root, testsize):
        ...
        self.pseudomasks = [pseudomask_root + f for f in os.listdir(pseudomask_root) if f.endswith('.jpg') or f.endswith('.png')]
        # self.pseudoedges = [pseudoedge_root + f for f in os.listdir(pseudoedge_root) if f.endswith('.jpg') or f.endswith('.png')]
        self.images = sorted(self.images)
        self.pseudomasks = sorted(self.pseudomasks)
        # self.pseudoedges = sorted(self.pseudoedges)
        ...
        self.pseudomask_transform = transforms.Compose([
            transforms.Resize((self.testsize, self.testsize)),
            transforms.ToTensor()])
        # self.pseudoedge_transform = transforms.Compose([
        #     transforms.Resize((self.testsize, self.testsize)),
        #     transforms.ToTensor()])
        self.size = len(self.images)
        self.index = 0
    ...
    def load_data(self):
        ...
        # pseudoedge = self.rgb_loader(self.pseudoedges[self.index])
        HH = image.size[0]
        WW = image.size[1]
        image = self.transform(image).unsqueeze(0)
        pseudomask = self.pseudomask_transform(pseudomask).unsqueeze(0)
        # pseudoedge = self.pseudoedge_transform(pseudoedge).unsqueeze(0)
